...
* Empty cells:
  ...
  * Default to `null` unless `empty_cells` overrides
* Row order:

  * Preserved as written in Markdown
* Rows:
...
Empty cell example (default = null):
...
tables:
  expenses:
    key: id
    columns: [id, category, net, note]
    empty_cells: null
---

| id | category | net | note |
|----|----------|-----|------|
| h1 | Hosting  | 100 |      |
| a1 | Ads      |     | ok   |
```
...
### Type Coercion (Non-Strict)

Type coercion is allowed only when it is unambiguous and lossless. Implementations **MUST** reject conversions that can change meaning or hide errors.

Allowed coercions:

| From   | To     | Rule (must match exactly)              |
| ------ | ------ | -------------------------------------- |
| string | number | `-?\d+(\.\d+)?`                        |
| string | bool   | `true` or `false` (lowercase)           |
...
If a lookup row or column is missing (e.g., `rates[Support].rate`), evaluation **MUST** fail.
...
Only aggregate references are allowed inside `{{ }}`.
...
* Type mismatches
* Missing columns
* Missing table rows or columns in `table_ref` lookups
...
All future versions **MUST** be backward compatible. If a change cannot be made backward compatible, it **MUST** be introduced as a new format name.
