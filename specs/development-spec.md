...
- If `report_tables` is present, generate rendered rows from `rows_from` after aggregate evaluation.
...
- In the current TypeScript implementation, non-finite numeric literals, inputs, and arithmetic results surface as `E_NUMBER` diagnostics.
