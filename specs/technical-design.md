...
- Escaping: `\{\{` and `\}\}` render literal `{{` and `}}` in text. Interpolation runs on the Markdown AST; apply escaping and interpolation only within non-code text nodes, leaving fenced/inline code untouched.
